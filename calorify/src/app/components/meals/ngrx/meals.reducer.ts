...
export const mealsReducer = createReducer(
    ...
    on(actions.finishLoadingDailyMeals, (state, { payload }) => ({
        ...state, breakfast: payload.breakfast, lunch: payload.lunch, dinner: payload.dinner, isLoading: false
    })),
    on(actions.cancelMealsObservables, (state) => ({ ...state, isLoading: true, breakfast: null, lunch: null, dinner: null, foods: [] })),
);
